...
public class BookShopCartController {
    ...
    public String handlerCartRequest(@CookieValue(value = "cartContents", required = false) String cartContents, Model model) {
        if (cartContents == null || cartContents.equals("")) {
            ...
            int sumAllBooks = booksFromSlug.stream().map(Book::getPriceWithDiscount).reduce(Integer::sum).orElse(0);
            int sumOldPrice = booksFromSlug.stream().map(Book::getPrice).reduce(Integer::sum).orElse(0);
    ...
    public String handleRemoveBookStatus(@PathVariable("slug") String slug,
